...
/*			
				...
				case 'circumstance_tree':
					$GLOBALS['TSFE']->page['title'] = $this->pi_getLL('tx_civserv_pi2_circumstance.circumstance_tree','Circumstances');
				...
				case 'usergroup_tree':
					$GLOBALS['TSFE']->page['title'] = $this->pi_getLL('tx_civserv_pi2_usergroup.usergroup_tree','Usergroups');
				...
				case 'organisation_tree':
					$GLOBALS['TSFE']->page['title'] = $this->pi_getLL('tx_civserv_pi2_organisation.organisation_tree','Organisation');
				...
				case 'organisation_list':
					$GLOBALS['TSFE']->page['title'] = $this->pi_getLL('tx_civserv_pi2_menuarray.organisation_list','Organisation A - Z');
...
/*
				...
				case 'circumstance':
					$GLOBALS['TSFE']->page['title'] = $this->pi_getLL('tx_civserv_pi2_service_list.circumstance','Circumstance');
				...
				case 'usergroup':
					$GLOBALS['TSFE']->page['title'] = $this->pi_getLL('tx_civserv_pi2_service_list.usergroup','Usergroup');
...
/*	 
	function serviceList(&$smartyServiceList,$abcBar=false,$searchBox=false,$topList=false) {
		...
		if($this->piVars[char]>''){
			 $GLOBALS['TSFE']->page['title'] .=': '.$this->pi_getLL('tx_civserv_pi2_service_list.abc_letter','Letter').' '.$this->piVars[char];
		}
		...
		if($this->piVars[mode]=='organisation')$smartyServiceList->assign('subheading',$this->pi_getLL('tx_civserv_pi2_service_list.available_services','Here you find the following services'));
...
*/	
 ...
 /*
	function getServiceListHeading($mode,$uid) {
		switch ($mode) {
			case 'service_list':
				$heading = $this->pi_getLL('tx_civserv_pi2_service_list.service_list','Service list');
				#$heading = $this->pi_getLL('tx_civserv_pi2_service_list.overview','Overview');
			...
			case 'online_services':
				$heading = $this->pi_getLL('tx_civserv_pi2_service_list.online_service_list','Online Services');
				if(!$this->conf['includeNameInHeading']){
					// test bk: premature return instruction to prevent that anything is added to the heading
					return $heading;
				}else{
			...
			case 'circumstance' :
				$heading = $this->pi_getLL('tx_civserv_pi2_service_list.circumstance','Circumstance');
				$field = 'nv_name';
				$table = 'tx_civserv_navigation';
				break;
			case 'usergroup' :
				$heading = $this->pi_getLL('tx_civserv_pi2_service_list.usergroup','Usergroup');
				$field = 'nv_name';
				$table = 'tx_civserv_navigation';
				break;
			case 'organisation' :
				$heading = $this->pi_getLL('tx_civserv_pi2_organisation_list.organisation_list','Organisation');
				// test bk: make it fitting for ms layout
				#$heading = $this->pi_getLL('tx_civserv_pi2_organisation_list.organisation_list.heading','Organisation');
				if($this->conf['includeNameInHeading']){
					$field = 'or_name';
		...
		if ($mode != 'service_list' && $mode != 'online_services') {
			...
			if (count($category) == 1) {
				...
				$GLOBALS['error_message'] = $this->pi_getLL('tx_civserv_pi2_error.unknown_category','The given Category (Circumstance, Usergroup or Organisation) is unknown.');
				return false;
			}
		} else {
			$heading .= ': ' . $this->pi_getLL('tx_civserv_pi2_service_list.overview','Overview');
...
/*	 
	function navigationTree(&$smartyTree,$uid,$searchBox=false,$topList=false) {
		...
		// Assign labels
		$smartyTree->assign('circumstance_tree_label',$this->pi_getLL('tx_civserv_pi2_circumstance.circumstance_tree','Circumstances'));
		$smartyTree->assign('usergroup_tree_label',$this->pi_getLL('tx_civserv_pi2_usergroup.usergroup_tree','Usergroups'));
		$smartyTree->assign('organisation_tree_label',$this->pi_getLL('tx_civserv_pi2_organisation.organisation_tree','Organisation'));
		
		// test b.k. introduce special Headings for city of Münster
		$smartyTree->assign('circumstance_tree_heading',sprintf($this->pi_getLL('tx_civserv_pi2_circumstance.circumstance_tree.heading','Circumstances'), $this->community['name']));
		$smartyTree->assign('usergroup_tree_heading',sprintf($this->pi_getLL('tx_civserv_pi2_usergroup.usergroup_tree.heading','Usergroups'), $this->community['name']));
		$smartyTree->assign('organisation_tree_heading',sprintf($this->pi_getLL('tx_civserv_pi2_organisation.organisation_tree.heading','Organisation Structure'), $this->community['name']));
		
		#$smartyOrganisationList->assign('subheading',$this->pi_getLL('tx_civserv_pi2_organisation_list.available_organisations','Here you find the following organisations'));
...
/*	 
	function organisation_list(&$smartyOrganisationList, $abcBar=false,$searchBox=false,$topList=false){
		...
		$smartyOrganisationList->assign('heading',$this->pi_getLL('tx_civserv_pi2_organisation_list.organisation_list.heading','Organisation'));
		$smartyOrganisationList->assign('subheading',$this->pi_getLL('tx_civserv_pi2_organisation_list.available_organisations','Here you find the following organisations'));
...
// organisation:
			...
			$orga_res = $GLOBALS['TYPO3_DB']->exec_SELECTquery(
				...
				'tx_civserv_employee.uid = ' . $row['em_uid'] . ' 
				 AND tx_civserv_position.uid = '.$row['po_uid'] .'
...
/*	 
	function formList(&$smartyFormList,$organisation_id=0,$abcBar=false,$searchBox=false,$topList=false,$orgaList=false) {
		...
		foreach ($all_forms as $single_form) {
			...
			$forms[$actual_category][$form_row_counter]['name'] = $this->pi_getEditIcon($single_form['name'],'fo_name',$this->pi_getLL('tx_civserv_pi2_form_list.name','form name'),$single_form,'tx_civserv_form');
			$forms[$actual_category][$form_row_counter]['descr'] = $this->formatStr($this->local_cObj->stdWrap($this->pi_getEditIcon(trim($single_form['descr']),'fo_descr',$this->pi_getLL('tx_civserv_pi2_form_list.description','form description'),$single_form,'tx_civserv_form'),$this->conf['fo_name_stdWrap.']));
		...
		// get heading
		$heading = $this->pi_getLL('tx_civserv_pi2_form_list.form_list','Forms') . ': ';
		...
		if ($organisation_id != 0) {
			...
			if ($row = $GLOBALS['TYPO3_DB']->sql_fetch_assoc($res)) {
				...
				$GLOBALS['error_message'] = $this->pi_getLL('tx_civserv_pi2_error.unvalid_organisation','An invalid organisation id was given.');
				return false;
			}
		} else {
			$heading .= $this->pi_getLL('tx_civserv_pi2_form_list.overview','Overview');
		}
		
		// test bk: ??? Münster???
		$GLOBALS['TSFE']->page['title'] = $heading;
		$smartyFormList->assign('heading',$heading);
		$smartyFormList->assign('subheading',$this->pi_getLL('tx_civserv_pi2_form_list.available_forms','Here you find the following forms'));
		$smartyFormList->assign('assigned_services',$this->pi_getLL('tx_civserv_pi2_form_list.assigned_services','The following services are assigned with this form'));
...
/*	 
	function do_search(&$smartySearchResult,$searchBox) {
		...
		if (!empty($searchString)) {
			...
			if ($rowcount == 0) {
				$GLOBALS['error_message'] = $this->pi_getLL('tx_civserv_pi2_search.no_results','No search results found!');
				return false;
			} else {
				...
				$smartySearchResult->assign('employee_label',$this->pi_getLL('tx_civserv_pi2_search_result.employee','Matching employees'));
				$smartySearchResult->assign('service_label',$this->pi_getLL('tx_civserv_pi2_search_result.service','Matching services'));
				return true;
			}
		}	// End query services
		...
		// Empty query
			...
			$GLOBALS['error_message'] = $this->pi_getLL('tx_civserv_pi2_search.empty_query','Empty query! Search string required.');
			return false;
		}
		// End empty query
	}
...
/*	 
	function calculate_top15(&$smartyTop15,$showCounts=1,$topN=15,$searchBox=false) {
		...
		$smartyTop15->assign('top15_label',$this->pi_getLL('tx_civserv_pi2_top15.top15','The 15 most frequently requested services'));
		$smartyTop15->assign('serviceinformation_label',$this->pi_getLL('tx_civserv_pi2_common.serviceinformation','Service information'));
		$smartyTop15->assign('frequently_visited_label',$this->pi_getLL('tx_civserv_pi2_common.frequently_visited','The following sites are visited frequently'));
...
*/
 ...
 /*
	function serviceDetail(&$smartyService,$searchBox=false,$topList=false, $continueAbcBarFromServiceList=false) {
		...
		$smartyService->assign('heading',$this->pi_getLL('tx_civserv_pi2_service_list.overview','Overview' ));
		...
		if (!array_key_exists($service_common['pid'],array_flip(explode(',',$this->community[pidlist])))) {
			...
			$smartyService->assign('external_service_label',$this->pi_getLL('tx_civserv_pi2_service.external_service','This service is provided and advised by') . ' ' . $article . $row['cm_community_name']);
		...
		if ($GLOBALS['TYPO3_DB']->sql_num_rows($res_common) == 0) {
			$GLOBALS['error_message'] = $this->pi_getLL('tx_civserv_pi2_service.error_valid','Service does not exist or is not available.');
			return false;
		}
		...
		for ($i = 0; $i < count($service_employees); $i++) {
			if($service_employees[$i]['address']==2){
				$service_employees[$i]['address_long'] = $this->pi_getLL('tx_civserv_pi2_organisation.address_female', 'Ms.');
			}elseif($service_employees[$i]['address']==1){
				$service_employees[$i]['address_long'] = $this->pi_getLL('tx_civserv_pi2_organisation.address_male', 'Mr.');
			}
			...
			$service_employees[$i]['email_form_url'] = htmlspecialchars($this->pi_linkTP_keepPIvars_url(array(mode => 'set_email_form',id => $service_employees[$i]['em_uid'],sv_id => $service_employees[$i]['sv_uid'],pos_id => $service_employees[$i]['po_uid']),1,1));
			$service_employees[$i]['employee_url'] = htmlspecialchars($this->pi_linkTP_keepPIvars_url(array(mode => 'employee',id => $service_employees[$i]['em_uid'],pos_id => $service_employees[$i]['po_uid']),1,1));
		...
		while ($row = $GLOBALS['TYPO3_DB']->sql_fetch_assoc($res_forms) ) {
			...
			if ($row[checkbox] == 1 && in_array(array('uid' => intval($row[uid])),$service_transactions)) {
				$service_forms[$row_counter]['url'] = $this->cObj->typoLink_URL(array(parameter => $row[url])) . '&tx_civserv_pi2[id]=' . $uid;
		...
		if ($service_common[sv_descr_short] != "") {
			...
			$descr_short = $this->pi_getEditIcon($descr_short,'sv_descr_short',$this->pi_getLL('tx_civserv_pi2_service.description_short','short description'),$service_common,'tx_civserv_service');
		} else {
		...
		$descr_long = $this->pi_getEditIcon($descr_long,'sv_descr_long',$this->pi_getLL('tx_civserv_pi2_service.description_long','Long description'),$service_common,'tx_civserv_service');
		...
		$image_descr = $this->pi_getEditIcon($image_text,'image_text',$this->pi_getLL('tx_civserv_pi2_service.image_text','Image description'),$service_common,'tx_civserv_service');
		...
		if ($service_common[sv_fees] != "") {
			...
			$fees = $this->pi_getEditIcon($fees,'sv_fees',$this->pi_getLL('tx_civserv_pi2_service.description_fees','Fees'),$service_common,'tx_civserv_service');
		} else {
		...
		if ($service_common[sv_documents] != "") {
			...
			$documents = $this->pi_getEditIcon($documents,'sv_documents',$this->pi_getLL('tx_civserv_pi2_service.description_documents','Necessary Documents'),$service_common,'tx_civserv_service');
		} else {
		...
		//Legal local
		$legal_local = $this->pi_getEditIcon($service_common[sv_legal_local],'sv_legal_local',$this->pi_getLL('tx_civserv_pi2_service.legal_local','Legal foundation (local)'),$service_common,'tx_civserv_service');
		...
		if ($service_common[sv_legal_global] != "") {
			...
			$legal_global = $this->pi_getEditIcon($legal_global,'sv_legal_global',$this->pi_getLL('tx_civserv_pi2_service.legal_global','Legal foundation (global)'),$service_common,'tx_civserv_service');
		} else {
		...
		$smartyService->assign('service_label',$this->pi_getLL('tx_civserv_pi2_service.service','Service'));
		$smartyService->assign('ext_service_label',$this->pi_getLL('tx_civserv_pi2_service.ext_service','This is an external service offered by'));
		$smartyService->assign('description_label',$this->pi_getLL('tx_civserv_pi2_service.description','Description'));
		$smartyService->assign('fees_label',$this->pi_getLL('tx_civserv_pi2_service.fees','Fees'));
		$smartyService->assign('documents_label',$this->pi_getLL('tx_civserv_pi2_service.documents','Necessary documents'));
		$smartyService->assign('forms_label',$this->pi_getLL('tx_civserv_pi2_service.forms','Forms'));
		$smartyService->assign('legal_label',$this->pi_getLL('tx_civserv_pi2_service.legal','Legal foundation'));
		$smartyService->assign('legal_local_label',$this->pi_getLL('tx_civserv_pi2_service.legal_local','Legal foundation (local)'));
		$smartyService->assign('legal_global_label',$this->pi_getLL('tx_civserv_pi2_service.legal_global','Legal foundation (general)'));
		$smartyService->assign('organisation_label',$this->pi_getLL('tx_civserv_pi2_service.organisation','Responsible organisational unit(s)'));
		if(count($service_employees)==1){
			if($service_employees[0]['address']==2){
				$smartyService->assign('contact_label',$this->pi_getLL('tx_civserv_pi2_service.contact_female','Contact person'));
			}else{
				$smartyService->assign('contact_label',$this->pi_getLL('tx_civserv_pi2_service.contact_male','Contact person'));
			}
		}else{
			$smartyService->assign('contact_label',$this->pi_getLL('tx_civserv_pi2_service.contact_plural','Contact persons'));
		}
		$smartyService->assign('employee_details',$this->pi_getLL('tx_civserv_pi2_organisation.employee_details','Jumps to a page with details of this employee'));
		$smartyService->assign('similar_services_label',$this->pi_getLL('tx_civserv_pi2_service.similar_services','Similar services'));
		$smartyService->assign('phone_label',$this->pi_getLL('tx_civserv_pi2_organisation.phone','Phone'));
		$smartyService->assign('email_label',$this->pi_getLL('tx_civserv_pi2_organisation.email','E-Mail'));
		$smartyService->assign('web_email_label',$this->pi_getLL('tx_civserv_pi2_organisation.web_email','E-Mail-Form'));
		$smartyService->assign('subnavigation_label',$this->pi_getLL('tx_civserv_pi2_service.subnavigation','Sub-navigation'));
		$smartyService->assign('link_to_section',$this->pi_getLL('tx_civserv_pi2_service.link_to_section','Jump label to section'));
		$smartyService->assign('serviceinformation_label',$this->pi_getLL('tx_civserv_pi2_common.serviceinformation','Service information'));
		$smartyService->assign('pages_related_topics_label',$this->pi_getLL('tx_civserv_pi2_service.pages_related_topics','Pages with related topics'));
		$smartyService->assign('top',$this->pi_getLL('tx_civserv_pi2_service.top','Top of page'));
		$smartyService->assign('link_to_top',$this->pi_getLL('tx_civserv_pi2_service.link_to_top','Jump label to the beginning of this page'));
	...
	function setDebitForm(&$smartyDebitForm) {
		...
		if ($this->piVars[id] > '') {
			...
			if ($GLOBALS['TYPO3_DB']->sql_num_rows($result) == 1) {
				...
				$GLOBALS['error_message'] = $this->pi_getLL('tx_civserv_pi2_debit_form.error_service','No debit form found for this service!');
				return false;
			}
			...
			if ($GLOBALS['TYPO3_DB']->sql_num_rows($result) == 1) {
				...
				$GLOBALS['error_message'] = $this->pi_getLL('tx_civserv_pi2_debit_form.no_results','Debit form uid not set in transaction configuration table!');
				return false;
			}
		}
		...
		//Assign template labels
		$smartyDebitForm->assign('debit_form_label',$this->pi_getLL('tx_civserv_pi2_debit_form.debit_form','Authorisation form for debit entries from bank accounts'));
		$smartyDebitForm->assign('serviceName_label',$this->pi_getLL('tx_civserv_pi2_debit_form.serviceName','Name of service, for which the debit authorisation is granted'));
		$smartyDebitForm->assign('serviceSelect_label',$this->pi_getLL('tx_civserv_pi2_debit_form.serviceSelect','Please select the service, for which the debit authorisation shall be granted'));
		$smartyDebitForm->assign('cashNumber_label',$this->pi_getLL('tx_civserv_pi2_debit_form.cashNumber','Cash number / personal key'));
		$smartyDebitForm->assign('bankName_label',$this->pi_getLL('tx_civserv_pi2_debit_form.bankName','Name of bank'));
		$smartyDebitForm->assign('bankCode_label',$this->pi_getLL('tx_civserv_pi2_debit_form.bankCode','Bank code'));
		$smartyDebitForm->assign('accountNumber_label',$this->pi_getLL('tx_civserv_pi2_debit_form.accountNumber','Account number'));
		$smartyDebitForm->assign('accountHolder_label',$this->pi_getLL('tx_civserv_pi2_debit_form.accountHolder','Name of account holder'));
		$smartyDebitForm->assign('firstname_label',$this->pi_getLL('tx_civserv_pi2_debit_form.firstname','First name'));
		$smartyDebitForm->assign('surname_label',$this->pi_getLL('tx_civserv_pi2_debit_form.surname','Surname'));
		$smartyDebitForm->assign('agreement_label',$this->pi_getLL('tx_civserv_pi2_debit_form.agreement','I agree, that all unsolicited data provided by me are stored for the purpose of fulfilling the duties of the city treasury.'));
		$smartyDebitForm->assign('optional_data_label',$this->pi_getLL('tx_civserv_pi2_debit_form.optional_data','Optional information'));
		$smartyDebitForm->assign('phone_label',$this->pi_getLL('tx_civserv_pi2_debit_form.phone','Phone'));
		$smartyDebitForm->assign('email_label',$this->pi_getLL('tx_civserv_pi2_debit_form.email','E-Mail'));
		$smartyDebitForm->assign('button_send_label',$this->pi_getLL('tx_civserv_pi2_debit_form.button_send','Send'));
		$smartyDebitForm->assign('button_reset_label',$this->pi_getLL('tx_civserv_pi2_debit_form.button_reset','Cancel'));
...
/*	 
	function checkDebitForm(&$smartyDebitForm) {
		...
		if (empty($cashNumber) || !is_numeric($cashNumber) || strlen($cashNumber) > 8) {
			$smartyDebitForm->assign('error_cashNumber',$this->pi_getLL('tx_civserv_pi2_debit_form.error_cashNumber','Please enter a valid cash number!'));
			$is_valid = false;
		}

		if (empty($bankName)) {
			$smartyDebitForm->assign('error_bankName',$this->pi_getLL('tx_civserv_pi2_debit_form.error_bankName','Please enter the name of your bank!'));
			$is_valid = false;
		}

		if (empty($bankCode) || !is_numeric($bankCode) || strlen($bankCode) > 9) {
			$smartyDebitForm->assign('error_bankCode',$this->pi_getLL('tx_civserv_pi2_debit_form.error_bankCode','Please enter a valid bank code!'));
			$is_valid = false;
		}

		if (empty($accountNumber) || !is_numeric($accountNumber) || strlen($accountNumber) > 9) {
			$smartyDebitForm->assign('error_accountNumber',$this->pi_getLL('tx_civserv_pi2_debit_form.error_accountNumber','Please enter a valid account number!'));
			$is_valid = false;
		}

 		if (empty($firstname)) {
			$smartyDebitForm->assign('error_firstname',$this->pi_getLL('tx_civserv_pi2_debit_form.error_firstname','Please enter the first name of the account holder!'));
			$is_valid = false;
		}

		if (empty($surname)) {
			$smartyDebitForm->assign('error_surname',$this->pi_getLL('tx_civserv_pi2_debit_form.error_surname','Please enter the surname of the account holder!'));
			$is_valid = false;
		}

		if (!($email && t3lib_div::validEmail($email))) {
			$smartyDebitForm->assign('error_email',$this->pi_getLL('tx_civserv_pi2_debit_form.error_email','Please enter a valid email address!'));
			$is_valid = false;
		}
		...
		if ($is_valid) {
			...
			$smartyDebitForm->assign('complete',$this->pi_getLL('tx_civserv_pi2_debit_form.complete','Thank you! Your data has been successfully stored in our system.'));
			return true;
