...
class tx_civserv_pi2 extends tslib_pibase {
	...
	//store some basic fields for queries...
	var $basic_tx_civserv_organisation = array();
	var $basic_tx_civserv_employee = array();
	var $basic_tx_civserv_position = array();
	var $basic_tx_civserv_employee_em_position_mm = array();
	...
	function main($content,$conf)	{
		...
		if (!$choose) {
			...
			//store some basic fields for queries... BEFORE the functions within the switch are called
			$this->basic_tx_civserv_organisation = array(	'tx_civserv_organisation.uid as or_uid', 
			...
			$this->basic_tx_civserv_employee = array(	'tx_civserv_employee.uid as em_uid',	
			...
			$this->basic_tx_civserv_employee_em_position_mm = array(	'tx_civserv_employee_em_position_mm.uid as ep_uid',
			...
			$this->basic_tx_civserv_position = array(	'tx_civserv_position.uid as po_uid',
...
//			skip check on array_key_exists as we do not expect doublettes here
			...
			$this->assembleEmployeeData($employees[$row['em_uid']], $row);
...
/*
				...
				// keep this. we might want to toggle it with the above solution via $conf
...
*/				
			...
			$em_pos_res = $GLOBALS['TYPO3_DB']->exec_SELECTquery(
					 implode(', ', $this->basic_tx_civserv_position).', '.
					 implode(', ', $this->basic_tx_civserv_employee_em_position_mm),
					'tx_civserv_employee, 
					 tx_civserv_position, 
			...
			while ($em_pos_row = $GLOBALS['TYPO3_DB']->sql_fetch_assoc($em_pos_res) ) {
				//is the actual position linked to any organisation?
				$em_orpos_res = $GLOBALS['TYPO3_DB']->exec_SELECTquery(
						 implode(', ', $this->basic_tx_civserv_organisation),
						'tx_civserv_employee_em_position_mm,
						 tx_civserv_organisation,
				...
				if ($GLOBALS['TYPO3_DB']->sql_num_rows($em_orpos_res) >= 1) {			
					while ($em_orpos_row = $GLOBALS['TYPO3_DB']->sql_fetch_assoc($em_orpos_res) ) {
						...
						$this->assemblePositionData($employees[$row['em_uid']]['em_organisations'][$em_orpos_row['or_uid']]['positions'][$em_pos_row['po_uid']], $em_pos_row);
						
						// hol die mitarbeiter-stellen-daten aus dem resultset
						$this->assembleEmployeePositionData($employees[$row['em_uid']]['em_organisations'][$em_orpos_row['or_uid']]['positions'][$em_pos_row['po_uid']], $em_pos_row);
		...
		while ($row = $GLOBALS['TYPO3_DB']->sql_fetch_assoc($res_employees) ) {
			...
			if($row['em_uid'] == $row['or_supervisor']){
				...
				if(array_key_exists($row['po_uid'], $organisations[$row['or_uid']]['or_chief'][$row['em_uid']]['em_positions'])){
					...
					//it is possible that the supervisor of an organisation also occupies another postion at this organisation
					$this->assemblePositionData($organisations[$row['or_uid']]['or_chief'][$row['em_uid']]['em_positions'][$row['po_uid']], $row);
					$this->assembleEmployeePositionData($organisations[$row['or_uid']]['or_chief'][$row['em_uid']]['em_positions'][$row['po_uid']], $row);
				}
...
//ABC-BAR:
	...
	function makeEmployeeListQueryOrUid($or_uid='all', $limit=true, $count=false) { //do not put the orcode in quotation marks!!
			$temp_empty_pos = array();
			foreach($this->basic_tx_civserv_position as $field){
				#$temp_empty_pos[] = '\'nix\''; //dont't!
				$temp_empty_pos[] = 0;
			}
			
			$temp_empty_emppos = array();
			foreach($this->basic_tx_civserv_employee_em_position_mm as $field){
				#$temp_empty_emppos[] = '\'nix\''; //dont't!
				$temp_empty_emppos[] = 0;
			}
			//diesmal werden die organisationen und die positionen mit abgefragt!!!
			$fields =   implode(', ', $this->basic_tx_civserv_organisation).', '.
					   'tx_civserv_organisation.or_supervisor, '.
						implode(', ', $this->basic_tx_civserv_employee).', '.
	   					implode(', ', $this->basic_tx_civserv_position).', '.
						implode(', ', $this->basic_tx_civserv_employee_em_position_mm);
			...
			if ($count){
				...
				SELECT '.
					   ...
					   'tx_civserv_organisation.or_supervisor, '.
						implode(', ', $this->basic_tx_civserv_employee).', '.
						implode(', ', $temp_empty_pos).', '.
						implode(', ', $temp_empty_emppos).'
				FROM 
				tx_civserv_employee,
				tx_civserv_organisation
				WHERE 
	...
	function makeAbcBar($query) {
		...
		$abcBar .= "\n";
		//$this->pi_getLL('tx_civserv_pi2_employee_list.hod','Head(s) of Department');
		$abcBar .= '<label for="orga_dropdown" class="invisible">'.$this->pi_getLL('tx_civserv_pi2_employee_list.searchform_or_dropdown_label','organisation').'</label>';
		$abcBar .= "\n";
#		$abcBar .= '<select name = "'.$this->prefixId.'[or_uid][]" id="orga_dropdown">';	//multi-select
		$abcBar .= '<select name = "'.$this->prefixId.'[or_uid]" id="orga_dropdown">';		//single-select
		$abcBar .= "\n";
		...
		if(($this->conf['pi2orgaListItemAllOrganisations'] || $this->piVars['or_uid'] == 'all') && $this->piVars['mode'] == 'employee_list_or_uid' && !$actual){ //kruecke
			...
			$or_name = $this->getOrganisationName(intval($this->piVars['or_uid']));
			$abcBar .= '<option class="level_1" selected="selected" value="'.intval($this->piVars['or_uid']).'">'.$or_name.'</option>';
		}else{
			$this->piVars['or_uid'] = 'mist'; //reset!
			$abcBar .= '<option class="level_1" value="">'.$this->pi_getLL('tx_civserv_pi2_employee_list.searchform_choose','please choose').'</option>';
		}
		$abcBar .= "\n";
		...
		//add search-option 'all' in the end
		if($this->piVars['or_uid'] !== 'all'){
			$abcBar .= '<option class="level_1" value="all">'.$this->pi_getLL('tx_civserv_pi2_organisation.all','all Organisations').'</option>';
		}
		...
		$abcBar .= '</select>';
		$abcBar .= "\n";
		$abcBar .= '<input type="submit" value="'.$this->pi_getLL('tx_civserv_pi2_employee_list.searchform_send','send').'" id="absenden" />';
		$abcBar .= "\n";
		$abcBar .= '</form>';
		$abcBar .= "\n";
		$abcBar .= '</div>';
		...
		if($this->piVars['mode'] ==  'employee_list_search'){
			...
			$abcBar .= '<input	type="text" 
								...
								size="10"  
								title="'.$this->pi_getLL('tx_civserv_pi2_employee_list.searchform_title','enter searchword here').'" 
								value="'.$this->pi_getLL('tx_civserv_pi2_employee_list.searchform_search_text','search...').'"  
								onblur="if(this.value==\'\')this.value=\''.$this->pi_getLL('tx_civserv_pi2_employee_list.searchform_search_text','search...').'\';" 
								onfocus="if(this.value==\''.$this->pi_getLL('tx_civserv_pi2_employee_list.searchform_search_text','search...').'\')this.value=\'\';" />';
		}
		$abcBar .= "\n";
		$abcBar .= '<input type="submit" name="empossearch"  value="'.$this->pi_getLL('tx_civserv_pi2_employee_list.searchform_submit','go!').'" />';
		$abcBar .= "\n";
		$abcBar .= '</form>';
		$abcBar .= "\n";
		$abcBar .= '</div>';	
	 ...
	 function make_or_choice($uid, $or_choice, $level){
		...
		if ($GLOBALS['TYPO3_DB']->sql_num_rows($result) > 0) {
			while ($row = $GLOBALS['TYPO3_DB']->sql_fetch_assoc($result)) {
				...
				//check if the given Organisation has any employees at all.....
				$em_row_count = 0;
				...
				if($em_row_count > 0){
					...
					$or_choice .=  '<option disabled="disabled" class="level_'.$level.'" value="'.$row['uid'].'">';
				}
...
//todo: comment this properly
	function assembleEmployeeData(&$employees, $row){
		// Anrede bestimmen
		if($row['em_address']==2){
			$employees['address_long'] = $this->pi_getLL('tx_civserv_pi2_organisation.address_female', 'Ms.');
		}elseif($row['em_address']==1){
			$employees['address_long'] = $this->pi_getLL('tx_civserv_pi2_organisation.address_male', 'Mr.');
		}
		// weitere Daten...
		$employees['title'] = $row['em_title'];
		$employees['name'] = $row['name']; //alias in makeEmployeeListQueryAZ, needed for generic makeAbcBar
		$employees['firstname'] = $row['em_firstname'];
		
		//wenn im Fullname des Mitarbeiters der aktuelle searchstring drin vorkommt, dann gib den Namen fettgedruckt aus
		...
		// Bild	holen
		$employees['em_imagecode']=""; // leere Initialisierung
		if ($employees['em_image'] != "") {
			$image = $employees['em_image'];
			$imagepath = $this->conf['folder_organisations'] . $this->community['id'] . '/images/';
...
//todo: comment this properly
 	function assembleOrganisationData(&$organisations, $row){
		$organisations['or_uid'] = $row['or_uid'];
		$organisations['or_index'] = $row['or_index'];
		$organisations['or_code'] = $row['or_code'];
		$organisations['or_name'] = $row['or_name'];
		$organisations['or_synonym1'] = $row['or_synonym1'];
	...
	function assembleEmployeePositionData(&$positions, $row){
 	...
 	function getOrganisationName($or_uid){
		$res = $GLOBALS['TYPO3_DB']->exec_SELECTquery(
			'tx_civserv_organisation.or_name',
			'tx_civserv_organisation',
			'tx_civserv_organisation.uid = '.intval($or_uid),
			'',
			'',
			''
		);
	...
	function check_searchword($string){
		...
		if(		$TYPO3_CONF_VARS['BE']['forceCharset'] == 'utf-8' ||
				$GLOBALS['TSFE']->metaCharset == 'utf-8' ||
				$GLOBALS['TSFE']->renderCharset == 'utf-8' ){
			$searchword_pattern = utf8_encode('/^[A-Za-z0-9äöüÄÖÜß\- ]*$/');
		}else{
	...
	function strip_extra($string){
		// Mobilé (Zentrum fuer clevere Verkehrsnutzung) => Mobile.html
