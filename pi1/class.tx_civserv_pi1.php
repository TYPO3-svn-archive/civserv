...
#		debug($query, 'query serviclist');
	...
	function navigationTree(&$smartyTree,$uid,$searchBox=false,$topList=false) {
		...
		// test b.k. introduce special Headings for city of M�nster
...
#		debug($all_forms, 'all_forms');
		...
		for($i=0; $i<count($all_forms); $i++){
			...
			$where= ' 1 '.
					...
					AND tx_civserv_form.uid = '.intval($all_forms[$i]['uid']); //just in case..
...
#		debug($all_forms, 'all forms nochmal');
		...
		if ($organisation_id != 0) {
			$res = $GLOBALS['TYPO3_DB']->exec_SELECTquery(
						...
						'tx_civserv_organisation',
						'tx_civserv_organisation.uid = ' . intval($organisation_id)); //just in case
		...
		if ($orgaList) {
			$res = $GLOBALS['TYPO3_DB']->exec_SELECTquery(
						...
						$this->cObj->enableFields('tx_civserv_organisation').
					 	 ...
					 	 ((intval($organisation_id) > 0)?' AND tx_civserv_organisation.uid != ' . intval($organisation_id) : ''),
	...
	function makeFormListQuery($char='all', $organisation_id=0, $orderByCategory=0, $limit=true, $count=false) {
		...
		$where =	'1 '.
					...
					$this->cObj->enableFields('tx_civserv_service').
				 	 ' AND tx_civserv_service.uid = tx_civserv_service_sv_form_mm.uid_local
				 	 AND tx_civserv_form.uid = tx_civserv_service_sv_form_mm.uid_foreign';
		if (intval($organisation_id) != 0) {
			$from  .=	', tx_civserv_organisation, 
						tx_civserv_service_sv_organisation_mm';
			$where .=	$this->cObj->enableFields('tx_civserv_organisation').
					 	' AND tx_civserv_service.uid = tx_civserv_service_sv_organisation_mm.uid_local
					 	 AND tx_civserv_organisation.uid = tx_civserv_service_sv_organisation_mm.uid_foreign
						 AND tx_civserv_organisation.uid = ' . intval($organisation_id);
		}
		...
		if (intval($this->piVars['id']) > 0) {
			...
			if ($GLOBALS['TYPO3_DB']->sql_num_rows($result) == 1) {
				...
				$res_forms = $GLOBALS['TYPO3_DB']->exec_SELECT_mm_query(
							...
							'tx_civserv_service_sv_form_mm',
							'tx_civserv_form',
							'AND tx_civserv_form.uid = ' . intval($debit_form_uid) . 
							$this->cObj->enableFields('tx_civserv_service').
							$this->cObj->enableFields('tx_civserv_form'),
							'',
							'name',	//ORDER BY
...
#			$this->piVars['community_id'] = $_SESSION['community_id'];
		...
		//test bk: city of M�nster: define first menu-item via $conf!
...
#		debug($GLOBALS['TSFE']->fe_user->user, 'feuser');
	...
	function replace_umlauts($string){
		...
		//eliminate:
		$string=str_replace(".", "", $string);			// 'B�cherei Zweigstelle Wolbecker Str.'				--> buecherei_zweigstelle_wolbecker_str.html
		$string=str_replace(" - ", "-", $string);		// 'La Vie - Begegnungszentrum Gievenbeck'				--> la_vie-begegnungszentrum_gievenbeck.html
		$string=str_replace("- ", "-", $string);		// 'Veterin�r- und Lebensmittel...'						--> veterinaer-und_lebensmittel.html
		$string=str_replace("-, ", " ", $string);		// 'Amt f�r Stadt-, Verkehrs- und Parkplatzplanung'		--> amt_fuer_stadt_verkehrs_und_parkplatzplanung.html
		$string=str_replace(",", "", $string);			// 'Ich, du, M�llers's Kuh'								--> ich_du_muellers_kuh.html
	...
	function strip_extra($string){
		// Mobil� (Zentrum f�r clevere Verkehrsnutzung) => Mobile.html
