...
function init() {
	...
	function menuConfig()	{
		...
		$this->res = $GLOBALS['TYPO3_DB']->exec_SELECTquery(
			'*',			 						// SELECT ...
			'pages',		 						// FROM ...
			'pid='.$uidModelService[cf_value].' AND !deleted AND !hidden', 		// AND title LIKE "%blabla%"', // WHERE...
			'', 									// GROUP BY...
			'title',   									// ORDER BY...
			'' 										// LIMIT to 10 rows, starting with number 5 (MySQL compat.)
			);
	...
	function getSelectForModelService()	{
		...
		$this->res = $GLOBALS['TYPO3_DB']->exec_SELECTquery(
			...
			''.$where_clause.' AND !deleted AND !hidden',								// AND title LIKE "%blabla%"', // WHERE...
			'', 											// GROUP BY...
			'ms_name',   											// ORDER BY...
			'' 												// LIMIT to 10 rows, starting with number 5 (MySQL compat.)
			);
	...
	function get_folders($startnode) {
		...
		$result = $GLOBALS['TYPO3_DB']->exec_SELECTquery(
			...
			uid',
			'pages',
			'pid = '.$GLOBALS['TYPO3_DB']->quoteStr($startnode,'pages').' AND !deleted AND !hidden',
			'',
			'title',
			'');
		$selectedUIDs = array();
