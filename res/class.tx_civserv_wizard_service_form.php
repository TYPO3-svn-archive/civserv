 ...
 *
 *
 *   75: class tx_civserv_wizard_service_form extends t3lib_SCbase
 *   91:     function init()
 *  296:     function main()
 *  388:     function getFormulars($letter)
 *  440:     function formular_selected($formular_uid)
...
/**
* This is an abc-wizard for choosing formulars for a service. Multiple formulars can be selected.
*/
class tx_civserv_wizard_service_form extends t3lib_SCbase {
...
function init() {
		...
		$this->content.='
			...
			<title>'.$LANG->getLL('tx_civserv_wizard_service_form.title').'</title>
	...
	// This JavaScript is primarily for RTE/Link. jumpToUrl is used in the other cases as well...
			...
			function insertElements(table, type, fp, close, options)	{	//
				...
				if (1=='.($this->pArr[0]&&!$this->pArr[1]&&!$this->pArr[2] ? 1 : 0).')	{
					...
					if (cur_selected_uid != \'\'){
						...
						alert("'.$LANG->getLL('tx_civserv_wizard_service_form.warning_msg_1').'");
					}
				} else {
					alert("'.$LANG->getLL('tx_civserv_wizard_service_form.warning_msg_2').'");
					if (close)	{
						parent.window.opener.focus();
						parent.close();
					}
			...
			function returnOptions(cur_selected_uid,cur_selected_name)	{	//
				...
				for(i=0;i<document.serviceform.selectedFormulars.length;++i) {
					num_removed=0;

						// Removes all selectorbox entries of current letter from current options.
					for (j=0;j<cur_uid_arr.length-num_removed;j++) {
						if (cur_uid_arr[j-num_removed]==document.serviceform.selectedFormulars.options[i].value) {
					...
					if(document.serviceform.selectedFormulars.options[i].selected == true) {
						if (selected_uid=="") {
							selected_uid=document.serviceform.selectedFormulars.options[i].value;
							selected_name=document.serviceform.selectedFormulars.options[i].label;
						} else {
							selected_uid=selected_uid+"|"+document.serviceform.selectedFormulars.options[i].value;
							selected_name=selected_name+"|"+document.serviceform.selectedFormulars.options[i].label;
						}
					}
				}
	...
	function main()	{
		...
		$script=basename(PATH_thisScript);
			...
			// Displays the abc to select all formulars beginning with
			// the selected letter.
		$this->content.='
		...
		if ($letter=='') {
			...
			';
				...
				// Gets all formulars beginning with the chosen letter and displays them in a selectorbox.
			$this->content.=$this->getFormulars($letter);

				// Displays a OK-Button to save the selected formulars.
			$this->content.='
					</td>
				</tr>
			</table>

			<input type="button" name="Return" value="'.$LANG->getLL('tx_civserv_wizard_service_form.OK_Button').'" onclick="return save_and_quit();">
		';
		}

			// Displays a Cancel-Button at the end of the Page to exit the wizard without changing anything.
		$this->content.='
		<input type="button" name="cancel" value="'.$LANG->getLL('tx_civserv_wizard_service_form.Cancel_Button').'" onclick="parent.close();">
		</form>
		</body>
		</html>
		';
	 ...
	 *
	 * @param	[type]		$letter:  the selected letter to show formulars beginning with this letter in the selectorbox.
	 * @return	[type]		...
	 * @@return	string		Selector box with formulars.
	 */
	function getFormulars($letter)	{
		...
		if ($letter != "other") {
			...
			$this->res = $GLOBALS['TYPO3_DB']->exec_SELECTquery(
				...
				'upper(left(fo_name,1))=\''.$letter.'\' AND !deleted AND !hidden',	// AND title LIKE "%blabla%"', // WHERE...
				'', 										// GROUP BY...
				'fo_name',   								// ORDER BY...
				'' 											// LIMIT to 10 rows, starting with number 5 (MySQL compat.)
				);
			...
			$this->res = $GLOBALS['TYPO3_DB']->exec_SELECTquery(
				'*',			 							// SELECT ...
				'tx_civserv_form',							// FROM ...
				'!(upper(left(fo_name,1))=\'A\') AND !(upper(left(fo_name,1))=\'B\') AND !(upper(left(fo_name,1))=\'C\') AND !(upper(left(fo_name,1))=\'D\') AND !(upper(left(fo_name,1))=\'E\') AND !(upper(left(fo_name,1))=\'F\') AND !(upper(left(fo_name,1))=\'G\') AND !(upper(left(fo_name,1))=\'H\') AND !(upper(left(fo_name,1))=\'I\') AND !(upper(left(fo_name,1))=\'J\') AND !(upper(left(fo_name,1))=\'K\') AND !(upper(left(fo_name,1))=\'L\') AND !(upper(left(fo_name,1))=\'M\') AND !(upper(left(fo_name,1))=\'N\') AND !(upper(left(fo_name,1))=\'O\') AND !(upper(left(fo_name,1))=\'P\') AND !(upper(left(fo_name,1))=\'Q\') AND !(upper(left(fo_name,1))=\'R\') AND !(upper(left(fo_name,1))=\'S\') AND !(upper(left(fo_name,1))=\'T\') AND !(upper(left(fo_name,1))=\'U\') AND !(upper(left(fo_name,1))=\'V\') AND !(upper(left(fo_name,1))=\'W\') AND !(upper(left(fo_name,1))=\'X\') AND !(upper(left(fo_name,1))=\'Y\') AND !(upper(left(fo_name,1))=\'Z\') AND !deleted AND !hidden',	// AND title LIKE "%blabla%"', // WHERE...
				'', 										// GROUP BY...
				'fo_name',   								// ORDER BY...
				'' 											// LIMIT to 10 rows, starting with number 5 (MySQL compat.)
				);
		...
		$menuItems=array();
			...
			// Removes all formulars from other mandants so that only
			// the formulars of the actual mandant are displayed in the
			// selectorbox.
		...
		return '<select name="selectedFormulars" size="10" multiple="multiple">'.implode('',$menuItems).'</select>
			   ';
	}//end getFormulars
	 ...
	 *
	 * @param	[type]		$formular_uid: the uid of a formular
	 * @return	[type]		...
	 * @@return	boolean
	 */
	function formular_selected($formular_uid) {
