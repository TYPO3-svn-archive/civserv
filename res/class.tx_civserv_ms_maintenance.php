...
class tx_civserv_ms_maintenance {
	...
	function transfer_ms($params){
		...
		if ($params['table']=='tx_civserv_model_service')	{
			$res = $GLOBALS['TYPO3_DB']->exec_SELECTquery('*', // Field list for SELECT
				'tx_civserv_model_service', // Tablename, local table
				'ms_mandant != 0 AND ms_approver_one != 0 AND ms_approver_two != 0 AND !deleted AND uid='.$params['uid'], // Optional additional WHERE clauses
			...
			while ($row = $GLOBALS['TYPO3_DB']->sql_fetch_assoc($res)) {
				//get the target-folder for the responsible modelservices
				$ms_res = $GLOBALS['TYPO3_DB']->exec_SELECTquery('cm_model_service_temp_uid', // Field list for SELECT
				...
				$exists_res = $GLOBALS['TYPO3_DB']->exec_SELECTquery('*', // Field list for SELECT
	...
	function show_mandants(&$params, &$pObj) {
		$res = $GLOBALS['TYPO3_DB']->exec_SELECTquery('cm_community_id, cm_community_name', // Field list for SELECT
			'tx_civserv_conf_mandant ', // Tablename, local table
